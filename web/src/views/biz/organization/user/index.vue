...
<template>
  <div class="main-box">
    ...
    />
    <div class="table-box">
      <ProTable ref="proTable" title="人员列表" :columns="columns" :request-api="bizUserApi.page" :init-param="initParam">
        <!-- 表格 header 按钮 -->
        <template #tableHeader="scope">
          <s-button v-auth="bizUserButtonCode.add" suffix="人员" @click="onOpen(FormOptEnum.ADD)" />
          <s-button
            v-auth="bizUserButtonCode.batchDelete"
            type="danger"
            plain
            suffix="人员"
        ...
        <template #operation="scope">
          <el-space>
            <s-button v-auth="bizUserButtonCode.edit" link :opt="FormOptEnum.EDIT" @click="onOpen(FormOptEnum.EDIT, scope.row)" />
            <s-button
              v-auth="bizUserButtonCode.delete"
              link
              :opt="FormOptEnum.DELETE"
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item v-auth="bizUserButtonCode.pwdReset" :command="command(scope.row, cmdEnum.ResetPassword)">{{
                    cmdEnum.ResetPassword
                  }}</el-dropdown-item>
                  <el-dropdown-item v-auth="bizUserButtonCode.grantRole" :command="command(scope.row, cmdEnum.GrantRole)">{{
                    cmdEnum.GrantRole
                  }}</el-dropdown-item>
                </el-dropdown-menu>
              </template>
...
import { bizOrgApi, bizUserApi, SysUser, SysRole, bizRoleApi, bizUserButtonCode } from "@/api";
...
import GrantResource from "@/views/sys/limit/role/components/grantResource.vue";
import GrantPermission from "@/views/sys/limit/role/components/grantPermission.vue";
...
async function onDelete(ids: string[], msg: string) {
  // 二次确认 => 请求api => 刷新表格
  await useHandleData(bizUserApi.delete, { ids }, msg);
  RefreshTable();
}
...
const userId = ref<number | string>(0); //人员id
...
function handleCommand(command: Command) {
  switch (command.command) {
    case cmdEnum.ResetPassword:
      useHandleData(bizUserApi.resetPassword, { id: command.row.id }, cmdEnum.ResetPassword);
      break;
    case cmdEnum.GrantRole:
      userId.value = command.row.id; //获取人员id
      bizUserApi.ownRole({ id: command.row.id }).then(res => {
        roleSelectorRef.value?.showSelector(res.data); //显示人员选择器
      });
      break;
  }
}
...
function handleChooseRole(data: SysRole.SysRoleInfo[]) {
  ...
  bizUserApi.grantRole(grantUser); //授权角色
}

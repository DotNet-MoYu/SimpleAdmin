...
<template>
  <div>
    <form-container v-model="visible" :title="`${positionProps.opt}岗位`" form-size="600px">
      <el-form
        ref="positionFormRef"
        :rules="rules"
      ...
      >
        <s-form-item label="所属组织" prop="orgId">
          <org-selector v-model:org-value="positionProps.record.orgId!" :org-tree-api="bizOrgApi.tree" :show-all="false" />
        </s-form-item>
        <s-form-item label="岗位名称" prop="name">
          <s-input v-model="positionProps.record.name"></s-input>
        </s-form-item>
        <s-form-item label="岗位分类" prop="category">
...
import { bizOrgApi, SysPosition, bizPositionApi } from "@/api";
...
const rules = reactive({
  ...
  orgId: [required("请选择所属岗位")],
  category: [required("请选择岗位类型")],
...
function onOpen(props: FormProps.Base<SysPosition.SysPositionInfo>) {
  ...
  if (props.record.id) {
    ...
    bizPositionApi.detail({ id: props.record.id }).then(res => {

...
import { SysRole, sysRoleApi } from "@/api";
...
const roleCategoryOptions = dictStore.getDictList(SysDictEnum.ROLE_CATEGORY); // 角色类型选项
...
function onOpen(props: FormProps.Base<SysRole.SysRoleInfo>) {
  ...
  if (props.record.id) {
    ...
    sysRoleApi.detail({ id: props.record.id }).then(res => {

...
public class DictService : DbRepository<DevDict>, IDictService
{
    ...
    public async Task<SqlSugarPagedList<DevDict>> Page(DictPageInput input)
    {
        var query = Context.Queryable<DevDict>()
            ...
            .WhereIF(input.ParentId != null,
                it => it.ParentId == input.ParentId || it.Id == input.ParentId)//根据父ID查询
    ...
    public async Task Delete(DictDeleteInput input)
    {
        ...
        //获取当前字典分类
        var category = dicts.Where(it => it.Id == input.Id).Select(it => it.Category)
            .FirstOrDefault();
        //如果是系统字典提示不可删除
    ...
    private async Task CheckInput(DevDict devDict)
    {
        ...
        //判断是否从存在重复字典名
        var hasSameLable = dicts.Any(it =>
            it.ParentId == devDict.ParentId && it.Category == devDict.Category
            && it.DictLabel == devDict.DictLabel && it.Id != devDict.Id);
        if (hasSameLable)
        {
            throw Oops.Bah($"存在重复的字典名称:{devDict.DictLabel}");
        }
        //判断是否存在重复字典值
        var hasSameValue = dicts.Any(it =>
            it.ParentId == devDict.ParentId && it.DictValue == devDict.DictValue
            && it.Id != devDict.Id);
        if (hasSameValue)
        {

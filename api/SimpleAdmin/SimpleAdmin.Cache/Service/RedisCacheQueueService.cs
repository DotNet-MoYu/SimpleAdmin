...
public partial class RedisCacheService
{
    ...
    {
        return _simpleRedis.GetRedisQueue<T>(key);
    }

    /// <inheritdoc/>
    public List<T> GetQueue<T>(string key, int Count = 1)
    {
        return _simpleRedis.GetQueue<T>(key, Count);
    }

    /// <inheritdoc/>
    public T GetQueueOne<T>(string key, int timeout = 1)
    {
        return _simpleRedis.GetQueueOne<T>(key, timeout);
    }

    /// <inheritdoc/>
    public async Task<T> GetQueueOneAsync<T>(string key, int timeout = 1)
    {
        return await _simpleRedis.GetQueueOneAsync<T>(key, timeout);
    }
    ...
    {
        return _simpleRedis.GetDelayQueue<T>(key);
    }
}
